...
test.describe('Sistema de Permissões', () => {
  ...
  test.afterAll(async ({ request }) => {
    ...
    if (testTenantId && (process.env.TEST_MODE === 'true' || process.env.NODE_ENV === 'test')) {
  ...
  test('usuário sem permissão não deve ver botão de criar processo', async ({
    ...
    const botaoCriar = page.locator('button:has-text("Criar"), button:has-text("Novo Processo"), button:has-text("Novo")');
    
    // O botão não deve estar visível OU deve estar desabilitado
    const isVisible = await botaoCriar.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('usuário com permissão deve ver botão de criar processo', async ({
    ...
    const isVisible = await botaoCriar.first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('logs de recusa devem aparecer no histórico de auditoria', async ({
    ...
    const hasContent = await Promise.race([
      page.waitForSelector('[class*="Card"]', { timeout: 5000 }).then(() => true),
      page.waitForSelector('table', { timeout: 5000 }).then(() => true),
    ]).catch(() => false);
    
    expect(hasContent || page.url().includes('/auditoria-permissoes')).toBe(true);
  });
});
