...
export function useKanban(boardId?: string | null) {
  const { data: boardsData, mutate: mutateBoards, isLoading: boardsLoading } = useSWR("boards-kanban", () => listBoards({ ativo: true }));

  const boards = boardsData?.success ? boardsData.boards : [];

  const { data: boardData, mutate: mutateBoard, isLoading: boardLoading } = useSWR(boardId ? ["board-detail", boardId] : null, () => getBoard(boardId!));

  const board = boardData?.success ? boardData.board : null;

  const { data: tarefasData, mutate: mutateTarefas, isLoading: tarefasLoading } = useSWR(boardId ? ["board-tarefas", boardId] : null, () => getTarefasPorBoard(boardId!));
