...
export async function handleGoogleCalendarCallback(code: string, state: string) {
  try {
    ...
    if (!session?.user?.id) {
      ...
      console.log(`[DEBUG] Usuário encontrado pelo state: ${usuario.email}`);
      ...
      console.log(`[DEBUG] Sessão diferente, mas usuário válido: ${usuario.email}`);
    }
    ...
    logger.info(`Google Calendar conectado para usuário ${session.user.id}`);
