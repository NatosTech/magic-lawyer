...
export default function NovoContratoPage() {
  ...
  const { dadosBancarios, isLoading: isLoadingDadosBancarios } = useDadosBancariosAtivos();
          ...
          <div className="space-y-4">
            ...
            >
              {dadosBancarios.map((conta: any) => (
                <SelectItem key={conta.id} textValue={`${conta.banco} - ${conta.titularNome}`}>
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold">{conta.banco}</span>
                      {conta.principal && <span className="text-xs bg-primary-100 text-primary-700 px-2 py-0.5 rounded">Principal</span>}
                    </div>
                    <span className="text-sm text-default-500">
                      Ag: {conta.agencia} - CC: {conta.conta}
                      {conta.digitoConta && `-${conta.digitoConta}`}
                    </span>
                    <span className="text-xs text-default-400">{conta.titularNome}</span>
                    {conta.chavePix && <span className="text-xs text-default-400">PIX: {conta.chavePix}</span>}
                  </div>
                </SelectItem>
              ))}
            </Select>
