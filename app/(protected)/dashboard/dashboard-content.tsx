...
export function DashboardContent() {
  ...
  const { getDashboardTitle, getDashboardDescription, getWelcomeMessage } = useProfileNavigation();
    ...
    <section className="mx-auto flex w-full max-w-5xl flex-col gap-8 py-12 px-3 sm:px-6">
      ...
      <Card className="border border-white/10 bg-background/70 backdrop-blur-xl">
        <CardHeader className="flex flex-col gap-2 pb-2">
          ...
          <p className="text-sm text-default-400">Recursos disponíveis para o seu perfil.</p>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {Object.entries(permissions).map(([key, value]) => (
              <div key={key} className="flex items-center gap-2 min-w-0">
                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${value ? "bg-success" : "bg-default-300"}`} />
                <span className="text-sm text-default-600 truncate">{key.replace(/([A-Z])/g, " $1").replace(/^./, (str) => str.toUpperCase())}</span>
              </div>
            ))}
          </div>
        </CardBody>
      ...
      <Card className="border border-white/10 bg-white/5">
        <CardBody className="flex flex-col gap-3 text-sm text-default-400 md:flex-row md:items-center md:justify-between">
          ...
          <Button as={NextLink} color="primary" href="/help" radius="full" className="flex-shrink-0">
