...
export default function DashboardFinanceiroPage() {
  ...
  const { metricas, grafico, honorarios, dadosBancarios, advogados, clientes, isLoading, error, mutate } = useDashboardFinanceiro(filtros);
        ...
        <Card className="border border-danger/30 bg-danger/10 text-danger">
          <CardBody className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-1">
              <p className="font-semibold">Não foi possível carregar o dashboard financeiro</p>
              <p className="text-sm text-danger/80">{(error as Error | undefined)?.message || "Tente atualizar a página ou recarregar os dados."}</p>
            </div>
        ...
        <motion.div className="space-y-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }}>
          ...
          <Card className="border border-white/10 bg-background/70 backdrop-blur-xl">
            <CardHeader className="flex flex-col gap-2 pb-2">
              ...
              <p className="text-sm text-default-400">Indicadores consolidados de receitas, despesas e performance</p>
            </CardHeader>
            <Divider className="border-white/10" />
            <CardBody>
          ...
          <Card className="border border-white/10 bg-background/70 backdrop-blur-xl">
            <CardHeader className="flex flex-col gap-2 pb-2">
              <h2 className="text-lg font-semibold text-white">Evolução das Parcelas</h2>
              <p className="text-sm text-default-400">Análise temporal das parcelas por status de pagamento</p>
            </CardHeader>
            <Divider className="border-white/10" />
            <CardBody>
              <GraficoParcelasComponent grafico={grafico} isLoading={isLoading} />
            </CardBody>
          </Card>
          ...
          <Card className="border border-white/10 bg-background/70 backdrop-blur-xl">
            <CardHeader className="flex flex-col gap-2 pb-2">
              <h2 className="text-lg font-semibold text-white">Honorários por Advogado</h2>
              <p className="text-sm text-default-400">Distribuição de honorários com controle de privacidade</p>
            </CardHeader>
            <Divider className="border-white/10" />
            <CardBody>
              <HonorariosAdvogado honorarios={honorarios} isLoading={isLoading} />
            </CardBody>
          </Card>
          ...
          <Card className="border border-white/10 bg-background/70 backdrop-blur-xl">
            <CardHeader className="flex flex-col gap-2 pb-2">
              <h2 className="text-lg font-semibold text-white">Resumo do Sistema</h2>
              <p className="text-sm text-default-400">Estatísticas gerais de contas, advogados e clientes</p>
            </CardHeader>
            <Divider className="border-white/10" />
            <CardBody>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="rounded-2xl border border-white/10 bg-background/40 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs uppercase tracking-[0.2em] text-default-400">Contas Bancárias</p>
                      <p className="truncate text-xl font-semibold text-white">{dadosBancarios.length}</p>
                      <p className="text-xs text-default-400">{dadosBancarios.filter((c) => c.principal).length} conta(s) principal(is)</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-background/40 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs uppercase tracking-[0.2em] text-default-400">Advogados Ativos</p>
                      <p className="truncate text-xl font-semibold text-white">{advogados.length}</p>
                      <p className="text-xs text-default-400">{honorarios.length} com honorários cadastrados</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-background/40 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs uppercase tracking-[0.2em] text-default-400">Clientes Ativos</p>
                      <p className="truncate text-xl font-semibold text-white">{clientes.length}</p>
                      <p className="text-xs text-default-400">Clientes ativos no sistema</p>
                    </div>
                  </div>
                </div>
              </div>
