...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (action) {
      case "migrate_all":
        console.log("[API] Iniciando migração de todas as notificações legadas...");
      ...
      case "migrate_single":
        if (!notificationId) {
          return NextResponse.json({ success: false, error: "notificationId é obrigatório para migrate_single" }, { status: 400 });
        }

        console.log(`[API] Migrando notificação ${notificationId}...`);
        await NotificationMigrationService.migrateLegacyNotification(notificationId);
      ...
      case "check_migration":
        if (!notificationId) {
          return NextResponse.json({ success: false, error: "notificationId é obrigatório para check_migration" }, { status: 400 });
        }

        const isMigrated = await NotificationMigrationService.isNotificationMigrated(notificationId);
