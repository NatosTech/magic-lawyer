...
const plans = [
  {
    ...
    price: "R$ 97",
    period: "/mês",
    features: ["Até 50 processos ativos", "Gestão de clientes", "Controle de prazos", "Documentos básicos", "Suporte por email", "Relatórios simples"],
  ...
  return (
    <section className="mx-auto flex w-full max-w-6xl flex-col gap-8 py-12">
      <header className="space-y-4 text-center">
        ...
        <p className={subtitle({ fullWidth: true })}>Escolha o plano ideal para o tamanho e necessidades do seu escritório. Todos os planos incluem suporte especializado e atualizações contínuas.</p>
      </header>

      <div className="grid gap-6 lg:grid-cols-3">
        {plans.map((plan) => (
          <Card key={plan.id} className={`border border-white/10 bg-background/70 backdrop-blur-xl relative ${plan.popular ? "ring-2 ring-primary" : ""}`}>
            {plan.popular && (
            ...
            <CardBody className="pt-4">
              <ul className="space-y-3 mb-6">
                {plan.features.map((feature, index) => (
                  <li key={index} className="flex items-start gap-2 text-sm text-default-400">
                    <span className="text-primary mt-1">✓</span>
                    {feature}
                  </li>
                ))}
              </ul>
              <Button as={NextLink} color={plan.color} href="/login" radius="full" size="lg" variant={plan.popular ? "solid" : "bordered"} className="w-full">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {features.map((feature, index) => (
              <div key={index} className="flex gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-2xl">{feature.icon}</div>
                <div>
      ...
      <Card className="border border-white/10 bg-primary/5">
        <CardBody className="text-center">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
            ...
            <Button as={NextLink} className="border-white/20 text-white" href="/about" radius="full" size="lg" variant="bordered">
              Falar com vendas
            </Button>
          </div>
        </CardBody>
