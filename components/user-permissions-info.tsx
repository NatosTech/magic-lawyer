...
import { Shield, CheckCircle, XCircle, AlertTriangle, Info, User, Mail, Phone, Camera, Key, Settings, Crown, Building2 } from "lucide-react";
...
export function UserPermissionsInfo({ className }: UserPermissionsInfoProps) {
  const [permissions, setPermissions] = useState<SelfEditPermissions | null>(null);
    ...
    <Card className={className}>
      <CardHeader className="flex flex-col gap-2 pb-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">Permissões de Auto-edição</h3>
            <p className="text-sm text-primary-300">O que você pode alterar em seu perfil</p>
          </div>
        </div>
      </CardHeader>
      ...
      <CardBody className="space-y-6">
        ...
        <div className="p-4 rounded-xl bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm font-medium text-primary-300">Seu Nível de Acesso</span>
          </div>
          <Badge color={getRoleColor(userRole) as any} variant="flat" size="lg" className="font-semibold">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-lg font-semibold text-white">Permissões Permitidas</h4>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {permissions.canEditRoleSpecificData && (
              <div className="flex items-center gap-3 p-3 rounded-lg bg-success/10 border border-success/20">
                ...
                <div>
                  <p className="text-sm font-medium text-white">Dados Profissionais</p>
                  <p className="text-xs text-success-300">Informações específicas do cargo</p>
                </div>
              </div>
            )}
            ...
            {permissions.canEditRole && (
              <div className="flex items-center gap-3 p-3 rounded-lg bg-success/10 border border-success/20">
                ...
                <div>
                  ...
                  <p className="text-xs text-success-300">Alterar cargo/função</p>
                </div>
              </div>
            )}
        ...
        <div className="p-4 rounded-xl bg-gradient-to-r from-default/10 to-default/5 border border-default/20">
          ...
          <p className="text-sm text-default-300">
            Suas permissões são baseadas no seu nível de acesso atual. Para alterar permissões ou acessar funcionalidades restritas, entre em contato com um administrador.
          </p>
        </div>
      </CardBody>
    </Card>
