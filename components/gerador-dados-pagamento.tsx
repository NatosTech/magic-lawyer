...
import { Card, CardBody, CardHeader, Button, Chip, Divider, Tabs, Tab, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Textarea, Tooltip } from "@heroui/react";
import { QrCodeIcon, CreditCardIcon, BanknoteIcon, Copy as CopyLucide, Check as CheckLucide, DownloadIcon } from "lucide-react";
    ...
    <>
          ...
          <Tabs aria-label="Opções de pagamento" color="primary">
                ...
                {dadosBancarios.chavePix ? (
                  <>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Tooltip content="Copiar chave PIX">
                            <Button isIconOnly size="sm" variant="light" onPress={() => handleCopy(dadosPix.chave, "Chave PIX")}>
                              {copiedField === "Chave PIX" ? <CheckLucide className="h-3 w-3 text-success" /> : <CopyLucide className="h-3 w-3" />}
                            </Button>
                          </Tooltip>
                        </div>
                    ...
                    <div className="flex gap-2">
                      <Button color="success" startContent={<QrCodeIcon className="h-4 w-4" />} variant="flat" onPress={gerarQrCodePix}>
                        Gerar QR Code PIX
                      </Button>
                      <Button
                        color="success"
            ...
            >
              <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Tooltip content="Copiar linha digitável">
                      <Button isIconOnly size="sm" variant="light" onPress={() => handleCopy(dadosBoleto.linhaDigitavel, "Linha Digitável")}>
                        {copiedField === "Linha Digitável" ? <CheckLucide className="h-3 w-3 text-success" /> : <CopyLucide className="h-3 w-3" />}
                      </Button>
                    </Tooltip>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Tooltip content="Copiar código de barras">
                      <Button isIconOnly size="sm" variant="light" onPress={() => handleCopy(dadosBoleto.codigoBarras, "Código de Barras")}>
                        {copiedField === "Código de Barras" ? <CheckLucide className="h-3 w-3 text-success" /> : <CopyLucide className="h-3 w-3" />}
                      </Button>
                    </Tooltip>
                  </div>
                ...
                <div className="flex gap-2">
                  <Button color="primary" startContent={<QrCodeIcon className="h-4 w-4" />} variant="flat" onPress={gerarQrCodeBoleto}>
      ...
      <Modal isOpen={modalOpen} size="2xl" onClose={() => setModalOpen(false)}>
        <ModalContent>
          ...
          <ModalBody>
            <Textarea readOnly minRows={10} placeholder="Dados do QR Code aparecerão aqui..." value={modalContent} />
          </ModalBody>
          <ModalFooter>
