...
export function HonorariosAdvogado({
  honorarios,
  isLoading,
}: HonorariosAdvogadoProps) {
  if (isLoading) {
    return (
      <div className="space-y-4">
        {Array.from({ length: 3 }).map((_, index) => (
          <div
            key={`honorario-skeleton-${index}`}
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-default-400">
          Não há honorários cadastrados para o período selecionado.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {honorarios.map((honorario, index) => {
          const percentual =
            totalHonorarios > 0
              ? (honorario.totalHonorarios / totalHonorarios) * 100
              : 0;
          const percentualRecebido =
            honorario.totalHonorarios > 0
              ? (honorario.honorariosRecebidos / honorario.totalHonorarios) *
                100
              : 0;
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-3">
                  <Avatar
                    ...
                    name={honorario.advogadoNome.charAt(0)}
                    size="sm"
                  />
                  <div>
                    <h4 className="font-semibold text-white">
                      {honorario.advogadoNome}
                    </h4>
                    <div className="flex items-center space-x-2">
                      <Chip
                        color={
                          honorario.visibilidade === "PUBLICO"
                            ? "success"
                            : "warning"
                        }
                        size="sm"
                      ...
                      <span className="text-xs text-default-400">
                        {honorario.contratosAtivos} contrato(s)
                      </span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-lg font-bold text-white">
                    {formatCurrency(honorario.totalHonorarios)}
                  </p>
                  <p className="text-xs text-default-400">
                    {formatPercent(percentual)} do total
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-2 bg-success/10 rounded-lg border border-success/20">
                  ...
                  <p className="text-sm font-bold text-success">
                    {formatCurrency(honorario.honorariosRecebidos)}
                  </p>
                </div>
                <div className="p-2 bg-warning/10 rounded-lg border border-warning/20">
                  ...
                  <p className="text-sm font-bold text-warning">
                    {formatCurrency(honorario.honorariosPendentes)}
                  </p>
                </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-success">
              {formatCurrency(
                honorarios.reduce((sum, h) => sum + h.honorariosRecebidos, 0),
              )}
            </p>
          </div>
          <div>
            ...
            <p className="text-lg font-bold text-warning">
              {formatCurrency(
                honorarios.reduce((sum, h) => sum + h.honorariosPendentes, 0),
              )}
            </p>
          </div>
          <div>
