...
exports.triggerRealtimeEvent = triggerRealtimeEvent;
...
async function getAblyClient() {
    ...
    if (!ablyClient && process.env.ABLY_API_KEY) {
        ablyClient = new ably_1.default.Realtime({ key: process.env.ABLY_API_KEY });
 ...
 */
async function publishToAbly(event) {
    try {
 ...
 */
async function fallbackToHttp(event) {
    try {
 ...
 */
async function publishRealtimeEvent(type, params) {
    const event = {
        type,
 ...
 */
async function triggerRealtimeEvent(params) {
