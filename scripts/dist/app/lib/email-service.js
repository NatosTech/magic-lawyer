...
exports.emailService = exports.getNotificacaoTemplate = exports.getBoasVindasTemplate = void 0;
const nodemailer_1 = __importDefault(require("nodemailer"));
const prisma_1 = __importDefault(require("@/app/lib/prisma"));
// =============================================
// EMAIL TEMPLATES
// =============================================
const getBoasVindasTemplate = (data) => {
    const especialidadesText = data.especialidades.length > 0
...
exports.getBoasVindasTemplate = getBoasVindasTemplate;
const getNotificacaoTemplate = (data) => {
...
exports.getNotificacaoTemplate = getNotificacaoTemplate;
// =============================================
// EMAIL SERVICE
// =============================================
class EmailService {
    resolveFromName(tenantId, fallbackName) {
        return prisma_1.default.tenantBranding
            .findUnique({ where: { tenantId } })
            .then((branding) => branding?.emailFromName || fallbackName || "Magic Lawyer");
    }
    async getTenantEmailCredential(tenantId, type = "DEFAULT") {
        const cred = await prisma_1.default.tenantEmailCredential.findUnique({
            where: { tenantId_type: { tenantId, type } },
        });
        if (!cred && type === "DEFAULT") {
            // tentar ADMIN como fallback
            return prisma_1.default.tenantEmailCredential.findUnique({
                where: { tenantId_type: { tenantId, type: "ADMIN" } },
            });
        }
        return cred;
    }
    createTransporter({ email, appPassword, }) {
    ...
    async sendEmailPerTenant(tenantId, emailData) {
        try {
            const credential = await this.getTenantEmailCredential(tenantId, emailData.credentialType || "DEFAULT");
            if (!credential) {
                return {
                    success: false,
    ...
    async sendBoasVindasAdvogado(tenantId, data) {
        const template = (0, exports.getBoasVindasTemplate)(data);
    ...
    async sendNotificacaoAdvogado(tenantId, data) {
        const template = (0, exports.getNotificacaoTemplate)(data);
    ...
    async testConnection(tenantId, type = "DEFAULT") {
        try {
    ...
    async getProvidersStatus(tenantId) {
